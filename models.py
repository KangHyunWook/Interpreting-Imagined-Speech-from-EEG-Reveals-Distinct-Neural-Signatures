...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        print('x:', x.shape, self.num_heads)
        exit()
...
class TransformerEncoder(nn.Sequential):
    def __init__(self, depth, config):
        ...
        super().__init__(*[TransformerEncoderBlock(config) for _ in range(depth)])
...
class Conformer(nn.Sequential):
    def __init__(self, config):
        ...
        super().__init__(
            TransformerEncoder(depth, config),
...
class LSTM(nn.Module):
    def __init__(self, config):
        ...
        self.hidden_size=hidden_sizes=[32, 16]
        rnn = nn.LSTM
        self.eeg_rnn1= rnn(14, hidden_sizes[0], bidirectional=True)
        ...
        self.project_eeg.add_module('project_eeg', nn.Linear(in_features=4*hidden_sizes[0],out_features=2))
